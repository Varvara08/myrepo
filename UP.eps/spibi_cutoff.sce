...
chi=0;
//
nf=655;//number of the latest file
//
kw=0;
for w=2.1:0.1:2.9
    kw=kw+1;
    ...
    for j=1:1:nf
        sig=0.001*j;
        s = s0 + 'CSBrush_mn' + string(w) + '_' + string(j) + '.dat';
        a=fscanfMat(s);
        [m,n]=size(a);
        x1 = a(1,7); //terminal group LB2
        x2 = a(2,7); //terminal group LB2 n100 (19); n50 (18); n200 ( )
        nmax=0;
        for k=3:m
            x3=a(k,7);
...
sout = s0 + 'yam_spibi_q' + string(q) + '_np' + string(np) + '_M_sigtr_sig1_sig2.dat';
