...
class brush():
    ...
    def __init__(self):
        #~ self.chi = chi
        self.Zbox =  int(self.n*1.5)
        self.rD = int( (8*pi*self.lB*self.Navogadro*self.cl*self.to_mols / 0.001)**-0.5 /self.distance)
        self.fname='brush_l_Z'+str(self.Zbox)+"_n"+str(self.n)+"_val"+str(self.val)+'_cl{:.2e}'.format(self.cl)+'_chi'+str(self.chi)+"_pK"+str(self.pK)+'_sig'+str(self.sigma)
    ...
    def inputfilegen(self):
        ...
        segments = ['poly','poly1','polyN']
        freedom  = ['free','pinned', 'free']
        ...
        composition = '(poly1)'+'(poly)'+str(self.n-2)+'(polyN)'
        ...
        output.append('mol : brush : theta : '+str(self.n*self.sigma))
        ...
        output.append('newton : isac : iterationlimit : 10000')
    ...
    def plotProfile(self, profile_name = 'sys : noname : potential'):
        ...
        # Plot polymer dencity #
        what_toPlot = 'ends'
        profile_toPlot = array(self.datadict['mol : brush : phi-polyN'])
        y_dataname = dataprefix+what_toPlot
    ...
    def getPressure(self):
        #~ try: self.datadict
        #~ except AttributeError: self.loadData()

        s_temp = deepcopy(self)
        #~ try: os.remove('data/'+s_temp.iguess_in)
        #~ except OSError: pass
        #~ s_temp.iguess_in = ''
        Rstar = self.Rstar
        RstarMax = self.RstarMax
        RstarMin = self.RstarMin


        #~ Rbox_range =  array(range(RboxMax, RboxMin,-1))
        if Rstar - 3 > RstarMin: R1 = Rstar - 3
        else: R1 = RstarMin
        if Rstar + 3 < RstarMax: R2 = Rstar + 3
        else: R2 = RstarMax

        Rstar_range =  arange(R1, R2)

        F = array([])
        V = array([])
        #~ print '###############', s_temp.cl*self.to_mols
        for i in range(len(Rstar_range)):

            s_temp.Rstar  = Rstar_range[i]
            s_temp.sfbox()

            Fvar = s_temp.datadict['sys : noname : Helmholtz energy(po)']
            Vvar = s_temp.datadict['lat : mylat : total number of lattice sites']
            F = append(F,Fvar)
            V = append(V,Vvar)

        R0 = s_temp.datadict['lat : mylat : offset_first_layer']

        DF_DRstar = dervec3(F)
        #~ DV_DRstar = dervec3(V)
        #~ DRstar_DV = DV_DRstar**(-1.)
        DRstar_DV = self.A * 1./4/pi*(V*3./4/pi + R0**3)**(-2./3)
        DF_DV = DF_DRstar * DRstar_DV

        # find index of Rbox
        ind = where(Rstar_range == Rstar)[0]
