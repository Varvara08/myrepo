...
class brush():
    ...
    def chainInPotention(self):
        if (len(array(self.datadict['mol : brush : phi']))) < (self.chain_length+5):
          phi_list = self.datadict['mol : brush : phi']
          for k in range(0,self.chain_length):
            phi_list.append(0.0)
        else:
          phi_list = self.datadict['mol : brush : phi']
        phi = array(phi_list)
        potention = -log(1-phi)
        #potention = potention_from_formula()
        #potention = change_potention_from_sfbox(potention)
        w = exp(-potention)
        print 'Calculation gtl and gfl...'
        calc_gtl_gfl(w)
        print 'Complete.'
        print 'Calculation phiL and zeL...'
        calc_phiL_zeL(w)
        print 'Complete.'
        print 'Complete.'
    ...
    def brushInPotention(self):
        ...
        calc_g2(w)
        print 'Complete.'
        print 'Calculation zbp and ze...'
        calc_zbp_ze(w)
        print 'Complete.'
        
    def calc_av_dis(self):
		#global mylistav, mylistdis
		av = 0; av2 = 0; dis = 0
		av=calc_average(zeL)
		av2=calc_2average(zeL)
		dis=sqrt(av2-av**2)
		mylistav.append(av)
		mylistdis.append(dis)
		return mylistav, mylistdis
    ...
    def plotProfile(self, profile_name = 'sys : noname : potential'):
        ...
        what_toPlot = 'phi_calc'+str(line_color_num)
        profile_toPlot = phiL
        ...
        xy.PlotLine.style.val = line_type
        ...
        y_axis.min.val = 1e-4
        x_axis.max.val = 150
        y_axis.label.val = '\italic{\\phi_{line}}'
        x_axis.label.val = '\italic{z}'

        ########################
        graph = graph1
        ## Plot counterions #
        what_toPlot = 'zbp_calc'+str(line_color_num)
        profile_toPlot = zbp_calc
        y_dataname = dataprefix+what_toPlot
        ...
        ########################
        graph = graph2
        # Plot the calc distribution of the branching points
        what_toPlot = 'zeL_calc'+str(line_color_num)
        profile_toPlot = zeL
        y_dataname = dataprefix+what_toPlot
        ...
        #y_axis.min.val = 1e-8
        x_axis.max.val = 150
        y_axis.label.val = '\italic{z_{eL}}'
        x_axis.label.val = '\italic{z}'

        ########################
        graph = graph3
        # Plot the calc distribution of the ends
        what_toPlot = 'ze_calc'+str(line_color_num)
        profile_toPlot = ze_calc
        y_dataname = dataprefix+what_toPlot
        ...
        ########################
        graph = graph4
        # Plot the calculated distribution of the branching points
        what_toPlot = 'V_calc'+str(line_color_num)
        profile_toPlot = potention
        y_dataname = dataprefix+what_toPlot
...
#        y_axis.log.val = True
    ...
    def plotAvDis(self, profile_name = 'sys : noname : potential'):
        global g, xy, y_axis
        #line_color=my_line_color()
        #line_type=my_line_type()
        #point_color_bp='#'+3*(hex(int((1-my_onefraction_bp[0])*255))[-2:])
        ...
        self.loadData()
        x_dataname = dataprefix+'sigma'
        g.SetData(x_dataname, mylist)

        ########################
        graph = graph0
        ## Average of ends #
        what_toPlot = 'av_calc'+str(line_color_num)
        profile_toPlot = array(mylistav)
        y_dataname = dataprefix+what_toPlot
        ...
        #x_axis.min.val = -0.6
        #x_axis.max.val = 0.1
        y_axis.label.val = '\italic{<Z_eL>}'
        x_axis.label.val = '\italic{\\sigma}'

        ########################
        graph = graph1
        ## Dispersion #
        what_toPlot = 'dis_calc'+str(line_color_num)
        profile_toPlot = array(mylistdis)
        y_dataname = dataprefix+what_toPlot
        ...
        #x_axis.min.val = -0.6
        #x_axis.max.val = 0.1
        y_axis.label.val = '\italic{\delta<Z_eL>}'
        x_axis.label.val = '\italic{\\sigma}'
