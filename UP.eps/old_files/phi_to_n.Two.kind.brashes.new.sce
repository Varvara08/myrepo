...
nf=1000;//number of the latest file
//nf=10;
i=1;
N=200;
for j=1:1:nf
...
//  v=j;
  ...
  for k=1:m
    ...
    b(k,5)=a(k,8)*1E6/w; // phiL_tot
    b(k,6)=a(k,18)*1E6; // nB1L (simular branch. point)
    b(k,7)=a(k,19)*1E6; //nB2L (end point)
...
i=i+1;
  ...
  sout = s0 + 'zav_q=' + string(q) +'_m='+ string(w) + '_np=' + string(np) + '.dat';
  u=file('open',sout,'unknown');
