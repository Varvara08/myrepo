...
puts $obs_file "\#Time     R_E         R_G         E_TOT       E_KIN"
...
for {set i 0} { $i <= $int_n_times } { incr i} {
    ...
    puts $obs_file [format "%.3e %.5e %.5e %.5e %.5e %.5e" $time $re $rg [lindex [lindex $energy 0] 1] [lindex [lindex $energy 1] 1] [lindex [lindex $energy [expr [llength $energy]-1]] 1] ]
    flush $obs_file
    if { $vmd_output=="yes" } { imd positions }
