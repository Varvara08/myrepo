...
dz1=sinh(x)/(2+cosh(x)); // степень растяжения стебля H_1=dz1*n
dz2=sinh(x/q)/(2+cosh(x/q)); // степень растяжения свободных ветвей H_2=dz2*n
...
f = f +(1.0-lambda)/(q+1) *((1+2*s+3*dz3)/6 *log(1+2*s+3*dz3) + (1+2*s-3*dz3)/6 *log(1+2*s-3*dz3) + 2/3 *(1-s)*log(1-s));// + вклад от растяжения длинного пути звезд нижней популяции
...
//s
dsddz3 = 3*dz3/sqrt(1.0+3.0*dz3^2);

//f(1)
dfedx = lambda*x/(q+1) * (ddz1dx + ddz2dx);
dfedx = dfedx + (1-lambda)/(q+1) * ((1/3 *dsddz3+0.5)*log(1+2*s+3*dz3) + (dsddz3/3 -0.5)*log(1+2*s-3*dz3) - 2/3 *dsddz3*log(1-s))*ddz3dx;
...
//f(2)
fa = (1+2*s+3*dz3)/6 *log(1+2*s+3*dz3) + (1+2*s-3*dz3)/6 *log(1+2*s-3*dz3) + 2/3 *(1-s)*log(1-s);
...
for k=1:n-1
  ...
  res(k,9)=obs(1); // dz1 
  res(k,10)=obs(2); // dz2
  res(k,11)=obs(3); //dz_av
  res(k,12)=obs(4); // phi1
  res(k,13)=obs(5); // phi2
  res(k,14)=obs(6); //phi_av 
  res(k,15)=obs(7);
  res(k,16)=obs(8);
  res(k,17)=obs(9);
  res(k,18)=obs(10);
...
s0 = '~/varvara/Documents/IMC/Analytical theory/Charged/add_ex/';
sout = s0 + 'lambda_vs_sig_q' + string(q) + '_v' + string(alpha) +'_v2.dat';
